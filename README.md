...
Runway is the presentation layer at the end of the Side-by-Side Blueprint pipeline:
...
- Interactive dependency graph with pan/zoom and rich modals
- Paper/PDF output with verification badges (optional)

## Installation

Add Runway as a dependency in your `lakefile.lean`:
...
Module names must be fully qualified (e.g., `MyProject.Chapter.Module`). Only declarations with `@[blueprint]` annotations are included.
...
| `Runway/Site.lean` | `NodeInfo`, `ChapterInfo`, `BlueprintSite` types |
| `Runway/Render.lean` | Dashboard rendering, node rendering |
| `Runway/DepGraph.lean` | Dependency graph page generation |
| `Runway/Theme.lean` | Page templates, sidebar generation |
| `Runway/Templates.lean` | Reusable HTML template components |
| `Runway/Paper.lean` | Paper HTML generation, metadata extraction |
| `Runway/Pdf.lean` | PDF compilation with multiple compilers |
| `Runway/Latex/Parser.lean` | LaTeX parsing |
| `Runway/Latex/Ast.lean` | AST types including `Preamble` |
| `Runway/Latex/ToHtml.lean` | LaTeX AST to HTML conversion |
| `Runway/Latex/ToLatex.lean` | LaTeX hook resolution |
| `Runway/Macros.lean` | MathJax macro extraction |
...
| `Runway/Dress.lean` | Dress artifact loading |
| `Runway/Traverse.lean` | `TraverseM` monad for loading artifacts |
| `Runway/DocGen4.lean` | doc-gen4 URL generation |
| `Runway/Assets.lean` | Asset copying utilities |

### Verso Genre

Runway defines a Verso Genre (`Blueprint`) for type-checked document authoring. The genre provides:

**Block extensions**:
- `node` - Embed a blueprint node by label
- `proof` - Embed just the proof of a node
- `code` - Highlighted code block
- `htmlDiv` - Wrapper div with CSS classes
- `depGraph` - Dependency graph visualization
- `theoremEnv` - Theorem/lemma/definition environment

**Inline extensions**:
- `leanRef` - Reference to a Lean declaration (links to docs)
- `math` - Inline LaTeX math
- `nodeRef` - Reference to a blueprint node
- `htmlSpan` - HTML span with CSS classes
...
```json
{
  ...
  "checkResults": { "connected": true, "cycles": [] },
  "keyDeclarations": ["thm-main", "thm-pnt"],
  "messages": [{"nodeId": "lem-helper", "message": "Needs refactoring"}],
  "projectNotes": { "blocked": [...], "potentialIssue": [...] }
}
```
...
Runway renders 6 status colors, matching the upstream Dress/LeanArchitect status model:
...
Runway depends on:
- [Dress](https://github.com/e-vergo/Dress) - Provides LeanArchitect, SubVerso, Verso
...
The Verso fork includes `SBSBlueprint` and `VersoPaper` genres for blueprint document authoring.

## Example Projects

### SBS-Test

Minimal test project (16 nodes, all 6 status colors):
...
| [Verso](https://github.com/e-vergo/verso) | Document framework (fork with SBSBlueprint genre) |
